...
contains
  ...
  ! ----------------------------------------------------------
   ! ----------------------------------------------------------
  subroutine compute_PlanckFrac(                        &
                    ncol, nlay, nbnd, ngpt,                &
                    nflav, neta, npres, ntemp, nPlanckTemp,&
                    fmajor, jeta, tropo, jtemp, jpress,    &
                    band_lims_gpt,           &
                    pfracin, gpoint_flavor, &
                    planckFrac) bind(C, name="compute_PlanckFrac")
    integer,                                    intent(in) :: ncol, nlay, nbnd, ngpt
    integer,                                    intent(in) :: nflav, neta, npres, ntemp, nPlanckTemp
    ! Interpolation variables
    real(wp),    dimension(2,2,2,nflav,ncol,nlay), intent(in) :: fmajor
    ...
    !$acc enter data copyin(fmajor,jeta,tropo,jtemp,jpress,pfracin,gpoint_flavor,one)
    !$acc enter data create(planckFrac)
    ! Calculation of fraction of band's Planck irradiance associated with each g-point
    ...
    !$acc exit data copyout(planckFrac)

  end subroutine compute_PlanckFrac
  !------------------------------------------------------------------------------------------  
