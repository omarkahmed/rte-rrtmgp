...
program rte_rrtmgp_clouds
  ...
  !$omp target enter data map(alloc:flux_up, flux_dn, bnd_flux_up, bnd_flux_dn)
  if(is_sw) then
    allocate(flux_dir(ncol,nlay+1))
    allocate(bnd_flux_dir(ncol,nlay+1,nbnd))
    !$acc enter data create(flux_dir, bnd_flux_dir)
    !$omp target enter data map(alloc:flux_dir, bnd_flux_dir)
  end if
  !
  ! Clouds
  !
  ...
  if(is_lw) then
    ...
    !$omp target exit data map(from:flux_up, flux_dn, bnd_flux_up, bnd_flux_dn)
    ...
    !$omp target exit data map(from:flux_up, flux_dn, flux_dir, bnd_flux_up, bnd_flux_dn, bnd_flux_dir)
