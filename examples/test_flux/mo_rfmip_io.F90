...
contains
  ...
  subroutine read_size(fileName, ncol, nlay, nexp)
    ...
    ! print *,'get_dim_size(ncid, "level")', get_dim_size(ncid, 'level')
    ...
    print *,'ncol_l = 0, nlay_l = 0, nexp_l = 0', ncol_l, nlay_l, nexp_l
  ...
  !
  subroutine read_and_block_gases_ty(fileName,  gas_names, names_in_file, gas_conc_array)
    ...
    print *,'read_and_block_gases_ty start'
    ...
    ! !
    ! gas_conc_temp_3d = read_field(ncid, "water_vapor", nlay_l, ncol_l) * read_scaling(ncid, "water_vapor")
    ! do b = 1, nblocks
    !   call stop_on_err(gas_conc_array(b)%set_vmr('h2o', gas_conc_temp_3d(:,b)))
    ! end do

    ! gas_conc_temp_3d = read_field(ncid, "ozone", nlay_l, ncol_l) * read_scaling(ncid, "ozone")
    ! do b = 1, nblocks
    ...
    do g = 1, size(gas_names)
      ...
      !
      if(string_in_array(gas_names(g), ['no2'])) cycle
      print *,'check on |', "vmr_" //trim(names_in_file(g)), '|', var_exists(ncid, "vmr_" // trim(names_in_file(g)))

      ! scale = read_scaling(ncid, "vmr_" // trim(names_in_file(g)))
      gas_conc_temp_3d = read_field(ncid, "vmr_" // trim(names_in_file(g)), nlay_l, ncol_l)
                     
      do b = 1, nblocks
        call stop_on_err(gas_conc_array(b)%set_vmr(trim(names_in_file(g)), gas_conc_temp_3d(:,b)))
      end do


    end do
    ncid = nf90_close(ncid)
    print *,'read_and_block_gases_ty done'
