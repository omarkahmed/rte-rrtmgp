...
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      SUBROUTINE DISORT_LW( NLYR, FRACSREV, PLNKLEV,
...
c    in RRTM
      ...
      REAL          fracsrev( maxcly), PLNKLEV(maxcly)
c     ..
c     .. Local Scalars ..
...
c     ..
      ...
      CHARACTER(len=18) ::      HNAMDIS,HVRDIS
...
C       DO LC = 1, NLYR
C         print *, LC, DTAUC(LC), SSALB(LC), PMOM(0:2, LC)
C       ENDDO
...
C             print *, lev, PKAG( LEV ), PLNKLEV(lev+1), 
C      &       (PKAG( LEV )/PLNKLEV(lev+1) -1 )*100.0
 40      CONTINUE
...
c                             **     cosines
         IF( FBEAM.GT.0.0 ) THEN
            ...
            CALL LEPOLYS( MAZIM, MXCMU, NSTR-1, ANGCOS, SQT, YLM0 )
...
c     ..
      ...
      PRINT *,"ALBEDO HAS NOt BEEN SUPPLIED"
      CALL EXIT(22)
...
c                                   ** STWL(58a)
   ...
   70    CONTINUE
      ...
      SUBROUTINE LEPOLYS( M, MAXMU, TWONM1, MU, SQT, YLM )
...
c      CALL ERRMSG( 'BDREF--Please supply a surface BDRF model', .TRUE. )
      ...
      REAL FUNCTION  BDREF_STD( WVNMLO, WVNMHI, MU, MUP, DPHI )
...
!                              ** Fig. 8.15 on page 231, expect for w.)
         ...
         BDREF_STD=W / 4. / (MU+MUP)*( (1.+B)* P + H0*H - 1.0 )

      ELSE

         CALL ERRMSG( 'BDREF_STD--Need to supply surface BDRF model'
     $     ,.TRUE.)

      END IF

      RETURN
      END FUNCTION  BDREF_STD
