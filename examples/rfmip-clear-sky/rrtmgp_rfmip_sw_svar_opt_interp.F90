...
program rrtmgp_rfmip_sw
  ...
  !
  real(wp) :: solcycfrac   ! Optional solar cycle fraction - use to interpolate variability indices
...
!  call load_and_init(k_dist, trim(kdist_file), gas_conc_array(1))
  call load_and_init(k_dist, trim(kdist_file), gas_conc_array(1), solar_var)
  if(.not. k_dist%source_is_external()) &
    stop "rrtmgp_rfmip_sw: k-distribution file isn't SW"
  nbnd = k_dist%get_nband()
  ngpt = k_dist%get_ngpt()

  ! Solar variability test - derive facular and sunspot indices by interpolation
  ! in solar fraction from mean solar cycle values
  solcycfrac = 0.025

  ! Derive facular and sunspot indices by interpolation from mean solar cycle at 
  ! requested solar cycle fraction
  call stop_on_err(solar_var%solar_var_index(solcycfrac, k_dist))
