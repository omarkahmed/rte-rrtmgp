...
extern "C" void sum_broadband(int ncol, int nlev, int ngpt, real *spectral_flux_p, real *broadband_flux_p) {
  ...
  // do ilev = 1, nlev
  //   do icol = 1, ncol
  parallel_for( Bounds<2>({1,nlev},{1,ncol}) , YAKL_LAMBDA ( int indices[] ) {
    int ilev, icol;
    storeIndices( indices , ilev,icol );
...
extern "C" void net_broadband_full(int ncol, int nlev, int ngpt, real *spectral_flux_dn_p, real *spectral_flux_up_p, real *broadband_flux_net_p) {
  ...
  //   do ilev = 1, nlev
  //     do icol = 1, ncol
  parallel_for( Bounds<3>({2,ngpt},{1,nlev},{1,ncol}) , YAKL_LAMBDA ( int indices[] ) {
    int igpt, ilev, icol;
